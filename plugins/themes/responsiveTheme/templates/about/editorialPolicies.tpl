...
<div id="focusAndScope"><h3 class="mycustom-background">{translate key="about.focusAndScope"}</h3>
...
<div id="sectionPolicies"><h3 class="mycustom-background">{translate key="about.sectionPolicies"}</h3>
{foreach from=$sections item=section}{if !$section->getHideAbout()}
	<h3>{$section->getLocalizedTitle()}</h3>
...
{if $currentJournal->getLocalizedSetting('reviewPolicy') != ''}<div id="peerReviewProcess"><h3 class="mycustom-background">{translate key="about.peerReviewProcess"}</h3>
...
<div id="publicationFrequency"><h3 class="mycustom-background">{translate key="about.publicationFrequency"}</h3>
...
<div id="openAccessPolicy"><h3 class="mycustom-background">{translate key="about.openAccessPolicy"}</h3>
...
<div id="authorSelfArchivePolicy"><h3 class="mycustom-background">{translate key="about.authorSelfArchive"}</h3> 
...
<div id="delayedOpenAccessPolicy"><h3 class="mycustom-background">{translate key="about.delayedOpenAccess"}</h3> 
...
<div id="archiving"><h3 class="mycustom-background">{translate key="about.archiving"}</h3>
...
{foreach key=key from=$currentJournal->getLocalizedSetting('customAboutItems') item=customAboutItem name=customAboutItems}
	{if !empty($customAboutItem.title)}
		<div id="custom-{$key|escape}"><h3 class="mycustom-background">{$customAboutItem.title|escape}</h3>
		<p>{$customAboutItem.content|nl2br}</p>
		{if !$smarty.foreach.customAboutItems.last}{/if}
		</div>
	{/if}
{/foreach}
